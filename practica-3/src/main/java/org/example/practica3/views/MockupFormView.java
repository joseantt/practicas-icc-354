...
@PermitAll
@Route(value = "projects/:projectId/create-mockup", layout = MainLayout.class)
@PageTitle("Create a Mockup | MockupAPP")
public class MockupFormView extends VerticalLayout implements BeforeEnterObserver {
    private Long projectId; // TODO: CHANGE WHEN IMPLEMENTING DEMIAN'S PART
    ...
    public MockupFormView(MockupService mockupService) {
    ...
    private void saveMockup() {
        ...
        Mockup mockup = Mockup.builder()
                ...
                .path(path.getValue())
                .accessMethod(accessMethod.getValue())
                .responseCode(responseCode.getValue())
                .contentType(contentType.getValue())
                .body(responseBody.getValue())
                .headers(headers)
                .responseTimeInSecs(responseTime.isEmpty() ? 0 : responseTime.getValue())
                .expirationTimeInHours(expirationTime.getValue())
                .build();
        // FALTA AGREGAR PROYECTO AL MOCKUP
        for(Header header : headers) {
            header.setMockup(mockup);
        }
        mockupService.save(mockup);
        UI.getCurrent().navigate(MainLayout.class); // TODO: CAMBIAR a vista de proyectos
    }
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        // Esto se inicia luego del constructor
    }
}
